...
export default function Stepper({
  ...
  const handleNext = async () => {
    ...
    if (!isLastStep) {
      // Mark next step as validated/accessible
              ...
              <React.Fragment key={stepNumber}>
                {renderStepIndicator ? (
                  ...
                  <StepIndicator
                    ...
                    isAccessible={validatedSteps.has(stepNumber)}
                    onClickStep={clicked => {
                      // Only allow navigation to validated steps
                      if (validatedSteps.has(clicked)) {
                        setDirection(clicked > currentStep ? 1 : -1);
...
function StepIndicator({ step, currentStep, onClickStep, disableStepIndicators, isAccessible }) {
  const status = currentStep === step ? 'active' : currentStep < step ? 'inactive' : 'complete';

  const handleClick = () => {
    if (step !== currentStep && !disableStepIndicators && isAccessible) {
      onClickStep(step);
    }
  };
  ...
  return (
    <motion.div 
      ...
      animate={status} 
      initial={false}
      style={{ cursor: isAccessible && step !== currentStep ? 'pointer' : 'default', opacity: isAccessible ? 1 : 0.5 }}
    >
      <motion.div
        variants={{
          inactive: { scale: 1, backgroundColor: '#222', color: '#a3a3a3' },
          active: { scale: 1, backgroundColor: '#ffc010', color: '#ffc010' },
          complete: { scale: 1, backgroundColor: '#ffc010', color: '#ffc010' }
        }}
...
function StepConnector({ isComplete }) {
  const lineVariants = {
    ...
    complete: { width: '100%', backgroundColor: '#ffc010' }
  };
